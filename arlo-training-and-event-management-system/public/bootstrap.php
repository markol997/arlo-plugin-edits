...
function set_title($title, $id = null, $meta = false) {
    ...
    if ($id === null || !in_array($id, $pages) || ($post && $id != $post->ID) || (!in_the_loop() && !$meta) || is_nav_menu_item($id))
        return ['title' => $title];

    if (!$cat && empty($arlo_location) && empty($arlo_search))
        return ['title' => $title];
...
function arlo_register_custom_post_types() {
    if(isset($_COOKIE['debug_gls']) || true){
        if(strpos($_SERVER['REQUEST_URI'], 'events') !== false && $_SERVER['REQUEST_URI'] != '/events/'){
            flush_rewrite_rules();
        }
        
        //echo 'ARLO BOOTSTRAP : ' . 'HERE #4';
    }
//return;

$shouldAlwaysGoHere = false;
 if(isset($_COOKIE['debug_gls'])){
        //$shouldAlwaysGoHere = true;
       // echo 'ARLO BOOTSTRAP : ' . 'HERE #4.1';
    }
    if (strpos($_SERVER['REQUEST_URI'], 'courses/view-course-info') !== false || $shouldAlwaysGoHere) {
    if(isset($_COOKIE['debug_gls'])){
        //echo 'ARLO BOOTSTRAP : ' . 'HERE #4.2';
    }
		$urlParts = explode('/', $_SERVER['REQUEST_URI']);
		if (!preg_match('/(month-|location-|eventtag-|templatetag-)/', $urlParts[3])) {
			$string = '/courses/view-course-info/'.$urlParts[3];
		} else {
			$string = '/courses/view-course-info/'.$urlParts[2];
		}

		$trimmedString = trim($string, '/');

        if ($_SERVER['REQUEST_URI'] == '/courses/view-course-info/') {
            return;
        }
		
		if ($_SERVER['REQUEST_URI'] == '/courses/view-schedule/') {
            return;
        }
		
		if ($_SERVER['REQUEST_URI'] == '/live-online-courses/continuous-learning/') {
            return;
        }

        if (!preg_match('/(month-|location-|eventtag-|templatetag-)/', $_SERVER['REQUEST_URI'])) {
            flush_rewrite_rules();
            return;
        } else {
            //@todo fixed filtered pages
            add_rewrite_rule('^' . $trimmedString . '/(region-([^/]*))?/?(cat-([^/]*))?/?(month-([^/]*))?/?(location-([^/]*))?/?(venue-([^/]*))?/?(delivery-([^/]*))?/?(eventtag-([^/]*))?/?(presenter-([^/]*))?/?(templatetag-([^/]*))?/?(state-([^/]*))?/?(page/([^/]*))?', 'index.php?page_id=' . $page_id . '&arlo-region=$matches[2]&arlo-category=$matches[4]&arlo-month=$matches[6]&arlo-location=$matches[8]&arlo-venue=$matches[10]&arlo-delivery=$matches[12]&arlo-eventtag=$matches[14]&arlo-presenter=$matches[16]&arlo-templatetag=$matches[18]&arlo-state=$matches[20]&paged=$matches[22]', 'top');
            flush_rewrite_rules();
        }

    }
if(isset($_COOKIE['debug_gls'])){
        //echo 'ARLO BOOTSTRAP : ' . 'HERE #6';
    }


    $settings = get_option('arlo_settings');
if(isset($_COOKIE['debug_gls'])){
        //echo 'ARLO BOOTSTRAP : ' . 'HERE #7';
    }
    foreach (Arlo_For_Wordpress::$post_types as $id => $type) {
if(isset($_COOKIE['debug_gls'])){
       // echo 'ARLO BOOTSTRAP : ' . 'HERE #7.1';
    }
...
function arlo_flush_permalinks() {
    ...
    //adrlo_register_custom_post_types();
    //flush_rewrite_rules();
}
